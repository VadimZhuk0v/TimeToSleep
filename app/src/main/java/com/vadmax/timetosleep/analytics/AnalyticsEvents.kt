package com.vadmax.timetosleep.analytics

object AnalyticsEvents {
    const val USER_ENABLE_PHONE_TIMER = "user_enable_phone_timer"
    const val USER_ENABLE_PC_TIMER = "user_enable_pc_timer"
    const val ENABLE_VIBRATION = "enable_vibration"
    const val ENABLE_SOUND_EFFECT = "enable_sound_effect"
    const val ENABLE_LOCK_SCREEN = "enable_lock_screen"
    const val OPEN_OPEN_SOURCE_LINK = "open_open_source_link"
    const val PAIR_PC = "pair_pc"
    const val UNPAIR_PC = "unpair_pc"
    const val TURN_OFF_PC = "turn_off_pc"
    const val SET_RINGER_MODE = "set_ringer_mode"
    const val SCAN_UNSUPPORTED_QR = "scan_unsupported_qr"
}
